                ...
                {classes.map(_class  => {
                    if(!_class.archived && !userInfo.archived_class.includes(_class._id)){
                        return <div className="class box box-shadow" key = {_class._id}>
                            ...
                            {_class.students.includes(userInfo._id)? <p className="box-option link" onClick = {() => Unenroll(_class._id)}>Unenroll</p>
                             :null}
                            {(_class.owner===userInfo._id )?<p className="box-option link" onClick = {() => window.location = `/class/${_class._id}/setting`}>Setting</p>
                             :null}
                        </div>
                    }else return null;
                })}
